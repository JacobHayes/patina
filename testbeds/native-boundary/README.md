...
|---|---|---|
| `native_abi` | Prefixed C crash/checkpoint and env protocol; POSIX fd/env/path ABI; C stdio at the edges of a run (teardown, a refusal, the first write, a failed `assert`); pipe/socketpair wakeups; reactor fields and exact virtual deadlines | Linux x86_64 + arm64, macOS; epoll/eventfd Linux-only, kqueue/nsec Darwin-only |
