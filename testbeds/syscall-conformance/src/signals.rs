//! Shared handler state for the signal-family probes (`use
//! syscall_conformance::signals as support;`): counting handlers that record
//! the signal, the `siginfo` sender fields and the tid they ran on, sigset
//! helpers, and bounded unobserved waits.
...
fn note(sig: c_int) {
    // Every fact is stored BEFORE the count moves: a probe on another thread
...
/// (unrecorded setup; the probe's checks say what it observed).
...
/// never happens fails the following check instead of hanging the leg.
pub fn wait_for_count(p: &crate::calls::Probe, n: usize) {
    p.rec.quiet(|| {
        for _ in 0..200 {
            if count() >= n {
                return;
