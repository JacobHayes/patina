...
mod scenario {
    use libc::*;
    use syscall_conformance::calls::{neg, Probe, StatView, TimeArg, AT_FDCWD};
    ...
    pub fn run(p: &Probe) {
        ...
        p.check(
            ...
            micro.atime_ns == 6000 * SEC + 123_456_000
                && micro.mtime_ns == 7000 * SEC + 654_321_000,
        );
        ...
        p.check(
            ...
            link_after.mtime_ns == link_times.mtime_ns
...
syscall_conformance::probe_main!("fs/times", scenario::run);
