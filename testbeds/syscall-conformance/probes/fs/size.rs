...
mod scenario {
    use libc::*;
    use syscall_conformance::calls::{neg, Probe, StatView, AT_FDCWD};
    ...
    pub fn run(p: &Probe) {
        ...
        p.check(
            ...
            p.fallocate(fd, FALLOC_FL_COLLAPSE_RANGE | FALLOC_FL_KEEP_SIZE, 0, 4096)
                == neg(EOPNOTSUPP),
        );
        p.check(
...
syscall_conformance::probe_main!("fs/size", scenario::run);
