...
| macOS frameworks (CoreFoundation / Security) | import audit → `macos-framework` (Apple-reserved prefixes `CF`/`kCF`/`Sec`/`kSec`), a named class over the bare `unknown-import` these `rustls-native-certs` / native-TLS-trust-root symbols fell to; the run refusal carries a determinism note (host keychain/trust store is mutable per-machine state) and the `--allow-unsupported-symbols` allow path | `native_gate_classifies_and_refuses_a_security_framework_symbol` (macOS e2e: a guest importing `SecTrustSettingsCopyCertificates` is refused, class + note named, and `audit` reports the same); `classifies_known_native_escape_symbols` (unit) | reclassification only sharpens an already-denied label — never relaxes a decision; a symbol coincidentally sharing a prefix would still be denied |
| Custom global allocator (pre-main deadlock) | **static defined-symbol scan** (`native_binary_installs_custom_global_allocator`: shim-linked binary defines `__rust_alloc` but omits the default-allocator `__rdl_alloc` shim) → BOTH `run` and `audit` refuse **before spawn** with a named `custom-global-allocator` diagnostic | `native_run_and_audit_refuse_a_custom_global_allocator` (e2e: default refuse; narrow `--allow-unsupported-symbols <list>` still refuses; `audit` refuses; blanket `--allow-unsupported-symbols all` warns + runs a non-deadlocking System-wrapper); `custom_global_allocator_detection_fails_closed_on_unparseable_input` (unit) | detection is structural, not behavioral: it refuses ALL custom global allocators (the safe ones too — the blanket `all` hatch is the opt-in), because a safe-vs-deadlocking distinction is not statically decidable and the failure it prevents is a silent pre-main hang |
...
**Pre-main-hang class — narrower-but-sound detection, general watchdog deferred.** The `custom-global-allocator` static refusal converts the *dominant* silent-pre-main-hang cause (a custom `#[global_allocator]` whose lazy init re-enters through the shim's allocating lock interposers — root-caused on tikv-jemallocator) into a named pre-spawn refusal, before any process exists. The fully-general class "guest reached no runtime op within N wall-seconds of exec" is NOT yet detected: a sound version needs a guest-liveness heartbeat from the shim (the `--starve` backstop's wall-clock kill loop is the pattern), because an unconditional wall-clock cap would false-positive on legitimately long native runs and virtual-time `--liveness-watchdog` never arms if the guest hangs before installing the runtime. That heartbeat touches the shim startup path (concurrent work), so it is deferred; the static allocator refusal is the standalone loud detector for the common sub-class in the meantime. Non-allocator pre-main hangs (e.g. a static initializer spinning on host time) remain a documented residual pending the heartbeat watchdog.
