...
| Thread lifecycle | import audit | C `escape_probe` (native-audit rejects `pthread_create` as `unmanaged-thread`) + classifier unit | `pthread_create` is interposed, so a shim-linked guest can only reach uninterposed thread creation through non-exported private stubs (not linkable) |
