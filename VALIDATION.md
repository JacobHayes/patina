...
- directory listings (format 13): every listed entry carries the inode it names (`getdents`'s `d_ino`), and a descriptor listing (`fs_read_directory_fd`) starts with `.` and `..`, the directory's own inode and its parent's. The fs-mem suite pins the listing (`a_path_only_open_names_a_location_and_a_plain_one_opens_the_entry`), and the `fs/dirent` conformance scenario checks every `readdir`/`readdir64` `d_ino` against `fstatat`;
