...
| Host introspection (macOS Mach/BSD/IOKit) | import audit → `host-introspection` (exact Mach/BSD names `sysctl`/`sysctlbyname`/`getrusage`/`task_info`/`mach_task_self_`/`mach_host_self`/`host_statistics64`/`host_processor_info`/`vm_page_size`/`vm_deallocate`/`proc_*` + IOKit entry-point prefixes `IOService`/`IORegistry`/`IOIterator`/`IOObject`), a named class over the bare `unknown-import` these `sysinfo`/`num_cpus`/hardware-inventory symbols fell to; the run refusal carries a determinism note (reads host CPU/memory/hardware/process state — nondeterministic across hosts and runs; interpose-or-refuse, never allowlist) and the `--allow-unsupported-symbols` path | `classifies_ecosystem_audit_symbol_batch` (unit — Mach/BSD/IOKit sample classifies; `sysctlbyname` stays denied; a user `IOWidget` does NOT match the IOKit prefixes; `strtoul` stays `unknown-import`); the sysinfo/mimalloc MREs audit with the class attached | reclassification only sharpens an already-denied label — never relaxes a decision; the IOKit match is namespace-prefix-scoped (not a bare `IO`) so it cannot capture arbitrary user `IO*` symbols |
