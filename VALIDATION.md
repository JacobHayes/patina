...
| Time | import audit **+** instruction scan (aarch64 `mrs CNTVCT_EL0`/`CNTVCTSS_EL0`, x86 `rdtsc`/`rdtscp`; on x86_64 Linux the two x86 forms are trap-managed rather than refused — `prctl(PR_SET_TSC)`, answered from the virtual clock, proved by `native_containment::tsc_reads_answer_from_virtual_clock`) | `native_run_prerun_gate_refuses_every_escape_class` (`time`) | the **Darwin commpage** time path: `mach_absolute_time`/`gettimeofday` fast paths read a kernel-mapped page with an ordinary `ldr`, **not** an `mrs`, so the instruction scan does **not** catch it — coverage comes from *interposing* `mach_absolute_time`/`clock_gettime`/`gettimeofday` (what libc/std actually call); a hand-rolled commpage reader that bypasses libc is an uncaught residual |
| Entropy | import audit **+** instruction scan (x86 `rdrand`/`rdseed`, aarch64 `mrs RNDR`/`RNDRRS`) | `native_run_prerun_gate_refuses_every_escape_class` (`arc4random`); `classifies_rdtscp_and_rdseed`, `classifies_aarch64_system_register_accesses` (patina-target); `native_containment::rdrand_is_refused_on_every_kernel` proves an `rdrand` guest stays refused on the very x86_64 Linux host where `rdtsc` runs trap-managed | novel entropy instruction encodings (no mechanism traps a hardware entropy read, so these are refuse-only — never downgradable) |
...
| Thread pointer | instruction scan (x86 `wrfsbase` and the FS selector loads `mov fs`/`pop fs`/`lfs`, aarch64 `msr tpidr_el0`); refused, no trap; glibc installs the pointer from ld.so, outside the scanned image | `classifies_thread_pointer_writes`, `classifies_aarch64_system_register_accesses`, `aarch64_thread_pointer_write_is_refused_by_name` (patina-target); `native_containment::thread_pointer_writes_are_refused_by_name` (planted guest, both arches) | encodings the decoder does not know; code in shared libraries the guest links against (the scan reads the executable only); code generated at run time |
| Far transfer (x86 compatibility-mode switch) | instruction scan (`lcall`/`ljmp` through memory, `lret`, `iret`; the direct forms `9a`/`ea` fail closed as undecodable); refused, no trap | `flags_real_forbidden_opcodes_at_a_boundary` (patina-target) | a switch to 32-bit code through `rt_sigreturn` with a rewritten saved CS, and code created at run time |
...
| Direct syscall (by name) | import audit (`syscall`) **+** instruction scan (`svc`/`syscall` opcodes, and x86 `int 0x80`/`sysenter`, which are never SUD-downgraded) | `native_run_prerun_gate_refuses_every_escape_class` (`syscall`); `flags_real_forbidden_opcodes_at_a_boundary`, `sud_manageability_is_instruction_direct_syscall_only` (patina-target) | — |
...
- signal generation (format 9): `signal_generated` records sequence, signal, process/task target, siginfo code and payload. `signal_operations_fixture_decodes_and_replays` pins the canonical encoding with the `format-13-signals.patina` feature fixture.
...
- the filesystem and memory families' operations (format 10): `fs_sync_all`, `fs_make_node`, `fs_rename_whiteout`, `fs_exchange`, the four xattr operations, and the page cache's and anonymous files' `fs_write_back_at` (what a shared mapping stored, written into its file; not refused by write seals, counted as a write for crash injection), `fs_create_anonymous` (`memfd_create`, with its hugetlb page size), `fs_seals` and `fs_add_seals` (`F_GET_SEALS`/`F_ADD_SEALS`), with the `socket` and `char_device` entry kinds. `format-13-memory.patina` pins the memory operations' canonical encoding; the `mem/mmap_file` and `mem/memfd` conformance scenarios record and replay them on every vehicle;
...
- the network family's operations (format 12): `net_bind_shared` (one member of an `SO_REUSEPORT` group), `net_connect` (a datagram socket pinned to its peer, `None` releasing it), `net_mark` (the type of service and the `IP_PKTINFO` source a socket's sends carry), a datagram's `dialed` address and `tos` mark (omitted when empty or zero), and the `unreachable` send disposition for a datagram nothing is bound to take. `network_operations_fixture_decodes_and_replays` pins their canonical encoding with the `format-13-network.patina` feature fixture, which must equal what a recording of the same decisions writes;
- directory listings (format 13): every listed entry carries the inode it names (`getdents`'s `d_ino`), and a descriptor listing (`fs_read_directory_fd`) starts with `.` and `..`, the directory's own inode and its parent's. The fs-mem suite pins the listing (`a_path_only_open_names_a_location_and_a_plain_one_opens_the_entry`), and the `fs/dirent` conformance scenario checks every `readdir`/`readdir64` `d_ino` against `fstatat`. The same format carries the descriptor inode lookup: `fs_fd_ino` answers the inode an open descriptor names, recorded like `fs_fd_path` with no modeled latency and no fault eligibility, because record and `flock` locks key on it and lock bookkeeping does no I/O. The same format carries signed timestamps: every metadata outcome's four times and the set-times operations' nanoseconds are signed 128-bit values, so a time before the epoch and one up to the volume's range (ext4's 1901-12-13 to 2446-05-10, which 64-bit nanoseconds cannot hold) cross the boundary unwrapped; the restart snapshot stores them the same way (snapshot version 7). The filesystem truncates a set time from either door (native or WASI) to its range as the kernel's `timestamp_truncate` does, ext4's for a named node and tmpfs's for an anonymous file (fs-mem `a_set_time_is_truncated_to_the_filesystems_range_never_refused_or_wrapped`), and the `fs/times` conformance scenario checks a pre-epoch time read back exactly and out-of-range times clamped on every vehicle;
...
| Privileged rows answered from the virtual credential (each row's pre-capability checks in the kernel's order, then its declared capability: the kernel's refusal without it, a named fatal with it; host-configuration rows from the declared `KERNEL_CONFIG`, never the host's sysctls) | shim `sud::privileged::tests::every_declaring_row_is_gated_on_its_declaration` (every registry row that declares capabilities has a case, or is listed unreachable; each case against a credential holding every capability but the row's declared ones — refused — and one holding just those — the granted fatal, at a declared capability; each declared capability changes some case's answer) and the bpf table test; `patina-dst-syscalls` `tests::capability_rows_are_routed`; conformance scenarios `fs/{mount,mount_api,open_tree}`, `sys/{admin,quota,ioport,root,hostname}`, `proc/{ptrace,namespaces}` through every vehicle, the libc one through the shim's own wrappers (`c/posix/privileged.c`), and `sys/{perf,bpf}`, `proc/seccomp`, `sys/landlock`, `sys/lsm`, `sys/keys`, `mem/userfaultfd` and `ipc/sysv_sem`'s undo through the kernel vehicles | Every privileged row a fatal `Trap(privileged)`, so a guest probing `mount`, `unshare` or `bpf` for support died where the kernel answers `EPERM`; the libc leg could not even resolve glibc's wrappers | **Class** | The declaration test fails a declaring row without a case, a check that consults a capability the row does not declare or lets a caller past without it, and a declared capability no case's answer depends on. It cannot catch a check and declaration that agree but are both wrong against the kernel; the scenarios can, for the unprivileged caller only. The conformance scenarios pin the answers and their order against the host kernel (authoritative on the pinned 6.8). Limits: the named fatals where the model ends (unsharing shared state from other threads, registering a `userfaultfd` range, non-array BPF map types, detaching a BPF program, `PTRACE_TRACEME`, entering seccomp strict mode or installing a filter, enforcing a Landlock ruleset, the key model's ends: other key types, nested keyrings, the thread, session and user keyrings, `request_key`'s upcall, collecting a revoked key or a keyring its last thread left, the unmodeled `keyctl` operations) are gaps where a scenario reaches them; `proc/namespaces` opens, reads the link of and `fstat`s `/proc/self/ns/uts` (read-only and `O_PATH`) and reaches `setns`'s namespace-type and capability checks through every vehicle; `fs/mount_query` runs without a gap through the kernel vehicles over the virtual mount table. |
...
Timestamps are signed: any second is accepted, a time before the epoch is kept,
