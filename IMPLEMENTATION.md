...
| 5 | trace hardening, crash FS, host capture, minimization, allocator support | V5 | Partial |
...
30. Memory and IPC (syscall-conformance arc, memory+ipc family). One mapping model behind the C `mmap`/`mmap64`/`munmap`/`mremap`/`msync`/`mprotect` interposers and the SUD rows (`crates/patina-native-shim/src/mem/`), replacing the C copy model whose `MAP_PRIVATE` test read `MAP_SHARED_VALIDATE`'s bit (every private file mapping was `EINVAL`) and the SUD row that trapped every descriptor but -1: a file mapping maps the file's page cache, a host memfd (`MAP_SHARED` directly, `MAP_PRIVATE` copy-on-write), coherent with `read`/`write`/`truncate`/`fallocate` through every descriptor (page-granular write-back before a read, mirroring after a change), durable at `msync(MS_SYNC)`/`fsync`, reloaded after an in-process crash; `do_mmap`'s refusals in order (`EACCES`, `ENODEV`, `EOPNOTSUPP` under `MAP_SHARED_VALIDATE`), `mprotect`'s `EACCES` on a shared view that may not write. `memfd_create` and seals are fs-mem anonymous nodes (driver operations `create_anonymous`, `seals`, `add_seals`, and `write_back_at`, which write seals do not refuse; trace format 10). Resource limits are a 16-resource virtual table (`src/limits.rs`) that `getrlimit`/`setrlimit`/`prlimit64` answer, and locking is bookkeeping against its `RLIMIT_MEMLOCK`, populated as `__mm_populate` does (Linux 5.14 for `MADV_POPULATE_*`); each page cache and segment is one host memfd, the host soft `RLIMIT_NOFILE` is raised to its hard limit at start, and a refused memfd is a named fatal; no hugetlb pages are configured (one answer across `mmap`, `shmget` and `memfd_create`); transparent huge pages are disabled at startup. System V shared memory, semaphores and message queues and POSIX message queues are modeled for one process and its threads (`src/thread/ipc.rs`), blocking on the scheduler, `EIDRM` on removal, `mq_notify` through the signal model; memory policy answers for one node (`src/numa.rs`); `mincore` and `remap_file_pages` pass through; `membarrier` is modeled. Gates: the `mem/*` and `ipc/*` conformance scenarios through every vehicle, with record/replay and the strace leak run; `native_abi`'s `mem_probe.c` modes (virtual limits under a zero host lock limit, `mprotect` refusals, hugetlb, THP, `SIGBUS` past the end, `mlock` populate refusals, the named fatal under a low host descriptor limit) and `mmap_crash_probe.c`; `mem/`, `numa.rs`, `thread/ipc.rs`, fs-mem and trace unit tests. The virtual CPU has neither protection keys nor user shadow stacks (so no answer depends on the host's CPU): the key rows answer as 6.8 without `OSPKE` (an empty allocation map: the first `pkey_alloc` `EINVAL`, later ones `ENOSPC`, `pkey_free` `EINVAL`, `pkey_mprotect` key -1 alone) and `map_shadow_stack` and `arch_prctl`'s shadow-stack codes as 6.8 without `USER_SHSTK` (`src/sud/mem.rs`; `ENOSYS` on aarch64, whose 6.8 builds neither), and `mem/pkeys` and `mem/shadow_stack` carry that difference from their host oracle as `ByDesign` gaps. `memfd_secret` makes a secret-memory file of the filesystem (`src/mem/memfd.rs`) whose bytes only a shared, locked, never-executable mapping reaches, the descriptor funnels refusing the rest as secretmem does (`process_madvise` answers through the self pidfd since the signals family added it); the residuals in `crates/patina-target/ESCAPE-CLASSES.md`.
...
31. Time, timers, scheduling and identity (syscall-conformance arc, time+timers+sched+identity family). Every clock id is decoded once (`crates/patina-native-shim/src/clocks.rs`, `clockid_to_kclock`/`pid_for_clock`), for the C `clock_gettime`/`clock_nanosleep` interposers and the SUD rows alike: `BOOTTIME`/`MONOTONIC_RAW` read the monotonic clock, `TAI` the realtime one, the coarse clocks the timekeeper at the virtual kernel's last tick (`HZ` 1000), the alarm clocks the realtime/boot time (the machine has an RTC; arming or sleeping on them needs `CAP_WAKE_ALARM`, `EPERM`), and the CPU clocks — static, `clock_getcpuclockid`'s and `pthread_getcpuclockid`'s — the virtual CPU time; `clock_getres` and `clock_nanosleep` answer each in the kernel's order (a clock with no sleep `EOPNOTSUPP`, the caller's own thread clock `EINVAL`, a sleep on CPU time the sleeper never spends a named refusal). **Virtual CPU time** starts at `STARTUP_CPU_NANOS` (`patina-dst-abi`, 1 ms, a model constant: the exec, loader and libc startup a Linux process has run before `main`) for the process and its main thread, and is then charged by the advance-on-spin rescue alone — a task observing the clock again and again at frozen virtual time — to the task the scheduler last picked (`Context::cpu_time_nanos`/`task_cpu_time_nanos`); sleeps, waits, injected latency, and a loop that computes without reading the clock are not charged. `times`, `getrusage` (C and SUD), the CPU clocks and the CPU-time timers read it. **Timers** (`src/thread/timers.rs`): `setitimer`/`getitimer`/`alarm`, the POSIX `timer_*` family (`SIGEV_NONE`/`SIGNAL`/`THREAD`/`THREAD_ID` through the signal model: `SI_TIMER`, the id, the value, overruns counted from the programmed expiry when a periodic timer's signal is dequeued, `TIMER_ABSTIME`) and timer descriptors (`FdKind::TimerFd`: expirations as one `u64`, `TFD_NONBLOCK`/`TFD_CLOEXEC`, `TFD_TIMER_ABSTIME`, `TFD_TIMER_CANCEL_ON_SET`, readiness through the reactor's waiter core; a reader blocked when the last descriptor closes keeps the timer, as the kernel's `read` holds the file). A timer expires when virtual time reaches it: at every boundary return, wherever a row reads a timer or the pending set, and — while every task waits — by advancing idle time to its deadline (`Context::advance_idle_to`, recorded as the deadlock rescue's `SleepUntil`); the advance-on-spin rescue never steps over it (`Context::set_alarm`), and while a CPU-time timer is armed each rescue advances toward it by what it still needs, up to the 1 ms ceiling, instead of ramping (`Context::set_cpu_alarm`), so a T ms CPU timer fires after ⌈T⌉ rescues; a task the deadlock rescue woke at its own deadline is settled before the timers due at that instant fire, and a signal generated as a wait ends is delivered at that call's return. An absolute realtime expiry maps onto the monotonic line as an absolute realtime sleep does (`Context::monotonic_deadline`). **Process tree and identity** (`src/identity.rs`): the guest is pid 2, the child of the pid namespace's init (pid 1, group and session 1, root's credential: uid and gid 0, every capability; no handlers); the guest starts leading its own group in init's session, and its group and session are state `setpgid`/`setsid` change by the kernel's rules (it may join group 1, after which it may start a session); `getpgid`/`getsid`/`capget`/`prlimit64`/the sched rows reach init too, and a check against init reads its credential: `prlimit64` of init is `EPERM` even to read, `capget` answers root's sets, a signal to init is `EPERM` (`check_kill_permission`; a `SIGCONT` within init's session passes and init drops it), so is changing its scheduling (`setpriority`, `sched_set*`, `ioprio_set`), `PRIO_USER` 0 names the guest's threads alone (while `ioprio_set`'s `IOPRIO_WHO_USER` 0 names uid 0, whose init refuses it: `EPERM`), `kill(-1)` reaches nobody (`ESRCH`), and ptrace-mode access to init (`move_pages`, `migrate_pages`, `pidfd_getfd`, `process_vm_*`) needs `CAP_SYS_PTRACE` (`EPERM`); thread ids sit one above task ids so the main thread's is the pid. `getresuid`/`getresgid`, the `set*id`/`setfs*id` rows and the C `setuid`/`setgid` answer as the kernel answers an unprivileged caller holding one identity (its own ids succeed and change nothing, anything else `EPERM`); `getgroups` answers its own group, `setgroups` is `EPERM`; `capget`/`capset` negotiate versions and hold the guest's empty sets; `uname` answers `Linux`, the run's `--hostname` (default `patina`; `gethostname` reads it through `uname`), a release at `VIRTUAL_ABI` and the machine, honoring a `UNAME26`/`PER_LINUX32` persona; `sethostname`/`setdomainname` are `EPERM`; `sysinfo` reports the 4 GiB virtual machine, the boot clock's uptime and init's and the guest's threads. The credential deny-traps of the process class (`setuid`/`setgid`/`setsid`/`setpgid`/`setgroups`) are models now on both platforms (XNU's rules on Darwin). **Scheduling** (`src/thread/sched.rs`): per-thread policy, nice, realtime priority, utilization clamps, I/O priority and persona, inherited at thread creation (`sched_fork`'s `SCHED_RESET_ON_FORK` rules), under `__sched_setscheduler`'s unprivileged checks against `RLIMIT_NICE`/`RLIMIT_RTPRIO`; `sched_setattr`/`sched_getattr` negotiate the struct size; the virtual machine has one CPU (`sched_*affinity`, `getcpu`, both doors). The clock-setting rows answer the unprivileged `EINVAL`-then-`EPERM` order; `adjtimex` reads an unsynchronized clock (`TIME_ERROR`); `syslog` is `EPERM` (`dmesg_restrict`). **Gates.** Every `time/*`, `sched/*`, `cred/*`, `sys/*` scenario (`sys/sysfs` over the filesystem types the virtual kernel registers, `volume::sysfs`), and `proc/ids`'s group and session rows, run without a gap, host-checked through every vehicle it has with record/replay and the strace leak run; `proc/ids` declares one by-design difference, `kill(-1, 0)`, which reaches no process in the two-process tree; runtime unit tests pin the startup cost, the CPU charge and the idle and spin alarms; shim unit tests pin the clock-id decode, the refusal orders, the credential, capability and process-tree rules, the forward arithmetic and the `SCHED_RESET_ON_FORK` inheritance; `native_workloads` pins a timer and a wait ending at one instant (an interval timer against `nanosleep`, a timer descriptor against `poll`) — both aborted before the rescue was settled first.

32. The filesystem libc symbol layer (syscall-conformance arc, fs family). glibc symbols whose kernel rows were already modeled on the raw-syscall door but which the shim did not define (registry `Absent`, so a guest importing one was refused by the pre-run audit) are now C wrappers over the same Rust entries, with glibc's userspace semantics on top. `statvfs`/`fstatvfs`/`statvfs64`/`fstatvfs64` convert the one statfs description as glibc's `internal_statvfs.c` does (`f_frsize` falling back to `f_bsize`, `f_fsid` packed from statfs's two words second-high, `f_favail` = `f_ffree`, `f_flag` = the mount flags without `ST_VALID`, and 2.39's `f_type` = statfs's). `posix_fadvise`/`posix_fadvise64` return the error number and leave errno alone. `getrlimit64`/`setrlimit64` share one definition with `getrlimit`/`setrlimit`, as glibc's do on a 64-bit target, and a NULL limit is 0 (glibc hands `prlimit64` NULL, which asks for and sets nothing) where the plain spellings used to dereference it. `copy_file_range`, `sendfile` and its LFS spelling `sendfile64` are the raw rows' transfer model (`src/transfer.rs`) with errno set. The twelve extended-attribute wrappers (by path, by link, by descriptor; the seven `l*`/`f*` spellings gained registry rows) are the rows' attribute model (`src/xattr.rs`), which names the path, the link or the descriptor explicitly, so a NULL path is `EFAULT` in the kernel's order (after the flags, name and size for set and remove, first for get and list). glibc's exported internal names `__open`/`__open64`/`__read`/`__write` are the plain calls, and its `_FORTIFY_SOURCE` file spellings (`__open_2`, `__open64_2`, `__openat_2`, `__openat64_2`, `__read_chk`, `__pread_chk`, `__pread64_chk`, `__readlink_chk`, `__readlinkat_chk`) keep glibc's abort semantics: a length past the buffer the compiler knew is `__chk_fail` ("*** buffer overflow detected ***: terminated") and an `__open*_2` with `O_CREAT` or `O_TMPFILE` is `__fortify_fail` naming the call, both a guest abort (SIGABRT, trace finalized) before any syscall; the fortify spellings join the `dlsym` routing table beside the network ones. The conformance scenarios that reached these symbols through `dlsym` now import them, and their gaps are gone, except `fs/fortify`'s abort legs, which fork a child each: under patina that fork is the process-lifecycle trap, a by-design stop, so the shim's aborts are asserted by `native_signals` instead.

33. Privileged rows answer the virtual credential (syscall-conformance arc, privileged family: capability-gated and host-configuration rows). The rows that were fatal `Trap(privileged)` answer as Ubuntu 24.04's 6.8 kernel answers an unprivileged caller. **Credential.** `identity::Credential` is the one credential (uid/gid, groups, the five capability sets as `Capability` masks); on Linux the raw and C id rows, `getgroups`, `capget` and `capset` (`cap_capset`'s rules against it, on the sets `mk_kernel_cap` cuts to the known capabilities; a change the rules would allow is a named fatal, the credential being fixed), the owner `stat` reports and `chown` (supplementary groups included), System V IPC ownership, `SO_PEERCRED`, `si_uid` and `PRIO_USER` read it. It is not yet an identity setting; ARCHITECTURE lists what one still needs. **Capability field.** `SyscallRow::capabilities` names the capabilities a row's kernel code checks; a row declaring any is routed (`capability_rows_are_routed`), and has a case in the shim's registry-derived declaration test (or is listed as unreachable: `quotactl`, `quotactl_fd`, `statmount`, `listmount`). **Checks.** `src/sud/privileged/` holds one check per row over the credential and the arguments, in the kernel's order, ending in `gate`: the refusal without the capability, `Unmodeled::Granted` with it; `privileged::answer` turns a granted capability or an unmodeled path into a named fatal. Mount (`mount`, `umount2`, `fsopen`, `fspick`, `fsmount`, `move_mount`, `mount_setattr`, `pivot_root`, `open_tree` with a clone, and without one the `O_PATH` open `openat` makes; `fsconfig` refuses every descriptor, as no filesystem context can exist), administration (`acct`, `vhangup`, `swapon`/`swapoff`, `reboot`, `kexec_load`/`kexec_file_load`, `init_module`/`finit_module`/`delete_module`, `sethostname`/`setdomainname`), quotas (`quotactl`/`quotactl_fd`: the device, `ENOTBLK`, and `ENOSYS` before any capability, the virtual machine having no block device or quota operations), `chroot` (the working directory's lookup, `paths::searchable_directory`), `iopl`/`ioperm`, and `setns` (a namespace file, `/proc/self/ns/*` in `src/nsfs.rs`: opened read-only a `FdKind::Namespace`, `O_PATH` a `FdKind::NamespacePath` that `fdget` refuses like every `O_PATH` kind, so `setns`, `fsync`, `ftruncate`, `getdents64`, `fchown`, `epoll_ctl` and the rest answer `EBADF`; of another type `EINVAL`, then the install's refusals: the caller's own user namespace `EINVAL`, a lone time namespace with threads `EUSERS`, `CAP_SYS_ADMIN` and for a mount namespace `CAP_SYS_CHROOT` `EPERM`; any other descriptor `EBADF` or `EINVAL`). **Declared configuration.** `patina_dst_syscalls::KERNEL_CONFIG` fixes the sysctls the host-configuration rows read — `perf_event_open` (`EACCES` without `CAP_PERFMON` or `CAP_SYS_ADMIN` at Ubuntu's paranoid 4), `bpf` (the size, copy and command checks; map creation's attribute and array-map checks, program flags and BTF loading, then `EPERM` at `unprivileged_bpf_disabled` 2; the id walks, by-id opens, task queries and statistics `EPERM` without `CAP_SYS_ADMIN`, program queries without `CAP_NET_ADMIN`; the object commands `EBADF`/`EINVAL`, the model holding no BPF object; a pinned-object lookup `EACCES`), `userfaultfd` (`EPERM` for kernel faults at `vm.unprivileged_userfaultfd` 0; a user-mode-only descriptor, `FdKind::Userfaultfd` in `src/mem/userfaultfd.rs`, answering the `UFFDIO_API` handshake once, reads and polls with nothing ever pending; as a file read-only, seeking nowhere, its inode the caller's), `ptrace` (`ESRCH`, `EIO`, `EPERM` for the own thread group and for init under Yama scope 1), `unshare` (`EINVAL`, `ENOSPC` with no user namespaces, `EPERM`; a lone thread's own state 0, `CLONE_SYSVSEM` applying its semaphore adjustments as `exit_sem` does) and `syslog` (`dmesg_restrict`). The IPC, pipe, descriptor and `listen` limits the shim answers from are fields of the same configuration. **libc.** The shim defines glibc's wrappers of these rows (`c/posix/privileged.c`), each entering the dispatcher; `chroot` is no longer a process-class deny-trap (on macOS it answers XNU's superuser-first `EPERM`, which a guest run on macOS 26.6 matched natively for an existing and a missing path). Named fatals where the model ends: a granted capability, unsharing filesystem state, descriptors or the undo list from other threads, registering a `userfaultfd` range or a range ioctl, a blocking `userfaultfd` read, a non-array BPF map type's checks, detaching a BPF program from its attach point, `open_tree` of a descriptor that names no filesystem entry, `PTRACE_TRACEME`. **Gates.** Shim `sud::privileged::tests` derive the declaring rows from the registry and hold each to its declared capabilities (a credential holding every other capability is refused, one holding the declared ones reaches the granted fatal, and each declared capability changes some case's answer), and a table test holds the bpf model's size, map-creation and command rules; the registry rule test; conformance `fs/mount`, `fs/mount_api`, `fs/open_tree`, `fs/mount_query` (`statmount`/`listmount` over the virtual mount table, `volume::MOUNTS`, with `statx`'s `STATX_MNT_ID_UNIQUE`), `sys/admin`, `sys/quota`, `sys/ioport`, `sys/root` and `proc/ptrace` run without a gap through every vehicle, `sys/perf` and `sys/bpf` through the kernel vehicles (they have no libc spelling), and `ipc/sysv_sem` checks `CLONE_SYSVSEM`'s undo; `mem/userfaultfd` runs without a gap, and `proc/namespaces` runs without a gap through every vehicle.

34. The rest of the libc layer (syscall-conformance arc, libc Phase C: the gaps the `proc/environ`, `fd/stdio`, `entropy/getentropy`, `time/localtime`, `time/libc_clocks`, `sys/nss`, `proc/dl`, `fd/termios` and `proc/wait` scenarios pinned). **stdio.** The sentinel streams buffer as glibc's libio does (`c/posix/stdio.c`): `stdout` fully buffered with the `st_blksize` of its descriptor (below BUFSIZ) or BUFSIZ, chosen lazily; `stderr` unbuffered; `_IO_new_file_xsputn`'s fill-write-direct-keep split for a long write; a write error surfacing at the flush with the buffer dropped; `puts`/`fputs` answering glibc's `len+1`/1; `fflush(NULL)` over both streams; a per-stream recursive scheduler mutex. `patina_shutdown` runs the flusher the POSIX constructor registers (`patina_register_stream_flusher`), so the atexit finalizer and a harness finalize write the buffer; the runtime's abort, fatal-signal and raw `exit_group` paths finalize through `shutdown_run`, without it. Every patina refusal, fatal and verdict abort flushes through one `flush_before_refusal`, which writes the stdout buffer (handed over by the salvage registered beside the flusher) after the captured stdout. After `main` returns the stream locks are not taken (`patina_in_teardown`): only the root task runs, and waiting on a lock a parked thread holds was a post-main scheduling refusal. **Buffering modes.** The stream state is libio's own (buffer, write area, `_IO_UNBUFFERED`/`_IO_LINE_BUF`/`_IO_CURRENTLY_PUTTING`/`_IO_ERR_SEEN`), and the write path is glibc 2.39's `_IO_new_file_xsputn`/`_IO_new_file_overflow`/`new_do_write`/`_IO_default_xsputn` over it, with the printf family putting through `__printf_buffer_to_file`'s rule (the stream's buffer while it has room, else 128-byte stages), so an unbuffered stream writes a long message in stages as glibc does. `setvbuf`, `setbuf`, `setbuffer` and `setlinebuf` (Linux) follow `_IO_setvbuf`/`_IO_setbuffer`, caller buffers and glibc's quirks included; `ferror`, `clearerr`, `flockfile` and `funlockfile` (both platforms) read the error flag and take the stream's recursive lock. Before, they ran libc's code on the sentinel `FILE` and crashed (the audit refused them as unknown imports). `native_abi` `buffering_modes_match_the_host` compares a report of every call's answer and the bytes on a pipe after it with the host's. **assert.** glibc's `__assert_fail` is a shim definition on Linux (registry `Modeled`, routed by `dlsym`): glibc's message (``PROGRAM: FILE:LINE: FUNCTION: Assertion `EXPR' failed.``, PROGRAM argv[0]'s basename) put into the stderr stream, then a guest `abort` (SIGABRT through the virtual kernel, the trace finalized). libc's own hook wrote through libc's `stderr`, the shim's sentinel in a guest, and died of SIGSEGV with the message lost; the audit no longer lists it known-safe, so an import of it (a link that lost the definition) is refused. `proc/exit` proves the exit flush: its last handler's final events go through C `stdout` unflushed, and a planted missing flush loses them (`event count: native 17, patina 15`). **Environment.** The store is the process's `environ`, not a runtime map (`c/posix/env.c`): glibc's `getenv` (the entry's own bytes), `__add_to_environ` (in-place overwrite, append, growing the array it allocated last or a copy of one it did not), `unsetenv` (every entry of the name, in place), `putenv` (the caller's string inserted, a bare name removed) and `clearenv` (its array freed, `environ` NULL), over whatever array `environ` names, an assigned one included. The mutators share a scheduler mutex (glibc's `envlock`), not taken after `main` returns (`patina_in_teardown`). The runtime keeps only the startup map it publishes and records, and two gates: `patina_env_read_gate` (NULL before the startup constructor finishes; the init-error and pre-harness-install aborts) and `patina_env_write_gate` (the pre-startup abort, `ENOSYS` without a runtime); `Context`'s mutation API and the per-thread `getenv` copy are gone. `putenv` is modeled (registry `Modeled`), which settles DECISIONS #4's open half. **Local time.** `localtime_r` is glibc's `tzset_internal`/`__tzset_parse_tz`/`compute_change`/`__tz_compute`/`__offtime` in `src/localtime.rs`: `TZ` and `TZDIR` read once (at the first call, as glibc's `localtime_r` does), the file glibc would open checked in the deterministic filesystem (a regular file there is a named fatal — TZif is not modeled — and its absence the fallback glibc takes: UTC for the default file or an empty name, the POSIX rule string otherwise, with the US rules when a daylight name has none), `int`-width year arithmetic where glibc's is `int`, and `EOVERFLOW` for a year past `int`. A unit test converts 19 rule strings at 20 instants (transitions, both hemispheres, `J`/`n`/`M` rules, negative and 167-hour transition times, the `int` year edges, `i64::MIN`/`MAX`) against the host glibc with `TZDIR` pointing nowhere and requires every field to match. **Name service.** `patina_dst_syscalls::PASSWD` is the virtual passwd database (the `ubuntu:24.04` image's 19 entries, root first, uid 1000 `ubuntu`; a registry test holds it to the identity), served by `patina_passwd_line`; `getpwuid_r` and the Linux `setpwent`/`getpwent`/`endpwent` (registry `Absent` → `Modeled`, so `sys/nss` imports them directly) parse each line into the caller's buffer as nss files does, `ERANGE` below the line's length plus newline, NUL and fgets' truncation marker; on Linux errno is left equal to the answer, 0 found or not (`sys/nss` records it). The native filesystem image seeds the identity's home, `patina_dst_syscalls::IDENTITY_HOME` (`/home/ubuntu` 0750 under `/home` 0755), which the registry test holds to the passwd entry. `__res_init`/`res_init` answer 0. **Unreachable spellings.** `clock_getres` (the row's `patina_clock_getres`), `__clock_gettime` (the `clock_gettime` body) and `__gettimeofday` (glibc's `gettimeofday`, which zeroes a time zone it is given — `gettimeofday` now does too) are defined, and `tcgetattr` is glibc's conversion over `TCGETS` through `patina_ioctl`, the row's entry, so a pipe or file answers `ENOTTY` and a closed number `EBADF`; `time/libc_clocks` and `fd/termios` import them directly. `fd/termios` checks the pipe and the closed number first and pins the machine's missing pseudoterminals (`/dev/ptmx` is `ENOSYS`) as its remaining gap. **getentropy.** A request past 256 bytes is `EIO` before anything is drawn (glibc's `GETENTROPY_MAX`), and `patina_entropy` — behind `getentropy`, `getrandom`, the `/dev/urandom` read and the raw row — copies the drawn bytes through `uaccess`, so a NULL or read-only buffer is `EFAULT` rather than `EINVAL` or a guest `SIGSEGV`. **wait4.** `sys_wait4` (every vehicle, the C `waitpid` included) refuses an option bit outside `WNOHANG|WUNTRACED|WCONTINUED|__WNOTHREAD|__WCLONE|__WALL` with `EINVAL` and a pid of `INT_MIN` with `ESRCH`, in `kernel_wait4`'s order before the childless `ECHILD`. **dlsym and dlerror.** The routing table is every name the shim defines as a libc contract — the registry's `Modeled`/`Partial` Linux rows, an X-macro list in `c/posix/dlsym.c` a lib test holds to the registry (red on a planted missing name) — each entry a hidden alias of the definition, so `dlsym(RTLD_DEFAULT, "getpid")` equals the program's `&getpid`, `dlsym("dlsym")` is `__wrap_dlsym`, and the doctrine's rule (never hand out a pointer that can be rebound to a public symbol) holds: a hidden alias is resolved when the shim is linked. `dlerror` is defined (registry `Modeled`): per thread, a failed lookup leaves `PROGRAM: undefined symbol: NAME` plus patina's reason, answered once, and a successful lookup clears it, as glibc's. Host-only names (`gnu_get_libc_version`, `__pthread_get_minstack`), the deny-trapped escapes and the control plane stay NULL; `dlopen`/`dlclose` stay refused. macOS keeps its entropy-pair table (no aliases there, and a guest's `dlsym` is not interposed). std guests are unaffected: on ELF, std's `weak!` probes (`statx`, `copy_file_range`, `getrandom`) are weak references the static link already binds to the shim's definitions, and the one name std resolves through `dlsym` (`__pthread_get_minstack`) is host-only and stays NULL. Dynamic lookup matters to the `getrandom` crate and to C callers of `dlsym`.
...
37. Privileged rows any caller may use (syscall-conformance arc, privileged family, class b). These rows were fatal `Trap(privileged)` though the kernel answers an unprivileged caller; each is now modeled in `src/sud/privileged/` in 6.8's order, stopping by name only where the process would be bound by something patina does not enforce. **seccomp** (`seccomp.rs`): the queries are answered (`SECCOMP_GET_ACTION_AVAIL` for 6.8's eight actions, `SECCOMP_GET_NOTIF_SIZES` 80/24/64); strict mode's and a filter's argument checks run in `do_seccomp`'s order, then without `no_new_privs` a filter needs `CAP_SYS_ADMIN` (`EACCES`); entering strict mode and installing a filter are named fatals, so no mode is ever entered and prctl's `PR_GET_SECCOMP` reads 0, while `PR_SET_SECCOMP` runs through the same checks. The program is never read: its copy and the classic-BPF checks (`bpf_check_classic`, `seccomp_check_filter`) come after the privilege check, so with `no_new_privs` a program 6.8 would refuse (`EFAULT`, `EINVAL`) ends at the install's named fatal too. `no_new_privs` is per thread, as the kernel's task flag is: a thread created after it is set inherits it, one that already existed does not. **Landlock** (`landlock.rs`, at `KERNEL_CONFIG`'s ABI 4, whose version and errata flags answer 4 and Ubuntu's 5): a ruleset is a descriptor kind, `FdKind::LandlockRuleset`, the anonymous `[landlock-ruleset]` inode (read-write, close-on-exec, read and write `EINVAL`, `ESPIPE`, `DEFAULT_POLLMASK` and `EPERM` at `epoll_ctl`), its handle the access rights it handles; `landlock_create_ruleset` copies the attribute as `copy_min_struct_from_user` does, `landlock_add_rule` checks a rule in 6.8's order (a parent on an internal filesystem is `EBADFD`) and accepts it without keeping it, and `landlock_restrict_self` needs `no_new_privs` or `CAP_SYS_ADMIN` (`EPERM`) before its flags and descriptor, past which enforcing is a named fatal. **LSM** (`lsm.rs`): `KERNEL_CONFIG.lsm_modules` declares the stack, capability, Landlock and Yama (100, 110, 105, in load order; the host's lockdown, integrity and AppArmor are its policy), which `lsm_list_modules` lists; none keeps a process attribute, so `lsm_get_self_attr` and `lsm_set_self_attr` answer `EOPNOTSUPP` (writing size 0) past `security_getselfattr`'s and `security_setselfattr`'s checks; every size is a `u32`, Ubuntu's backport of 6.9's fix. **Keys** (`keys.rs`): the guest's own keys, its process keyrings (made on demand, `_pid`, `KEY_POS_ALL | KEY_USR_VIEW`) and the `user` keys linked in them (`3f010000`). A process keyring is per-thread credential state, as in 6.8: the thread that makes one holds it, and so do the threads created after (`copy_creds`); a thread that existed before has none (`ENOKEY`) and makes its own on demand. A thread possesses its keyring and the keys in it; for any other key `key_task_permission` applies (the default mask leaves a non-possessor view only: `EACCES` to read, update, search or revoke it). A keyring whose last thread exited, and its keys, are the collector's at a time the model does not know, so naming them, or a quota answer their charge decides, is a named fatal. The guest has a session keyring outside the model, as a process Ubuntu starts has (`pam_keyinit`, systemd's `KeyringMode=`): naming it is a named fatal and nothing the guest can find is in it, so `request_key` of a revoked key's description answers the real host's `ENOKEY`, not the `EKEYREVOKED` a process with no session keyring gets. Serials in sequence from a fixed start; the quota of `KERNEL_CONFIG.keys_maxkeys`/`keys_maxbytes` over descriptions, payloads and 4-byte links (`EDQUOT`); `add_key` in 6.8's order (updating a live key of the same description in place, displacing a revoked one), `request_key` searching the process keyring (a miss without callout `ENOKEY`), and `keyctl`'s `GET_KEYRING_ID`, `UPDATE`, `REVOKE`, `CHOWN` (`EACCES` without `CAP_SYS_ADMIN` for another user or a group the caller is not in), `DESCRIBE`, `READ` (the length, copied only when it fits) and `CAPABILITIES` (the pinned configuration's two bytes, the rest of the room cleared); an operation past 32 is `EOPNOTSUPP`. Named fatals: other key types, nested keyrings, the thread, session and user keyrings, `request_key`'s upcall, revoking a keyring, reading a keyring of more than one key (its order is the associative array's), a keyring its last thread left, the collector (any key call once a revoked key is past `keys_gc_delay`, however much later: removing it is not modeled), and the other `keyctl` operations. **Gates.** `proc/seccomp` (its prctl door included), `sys/landlock`, `sys/lsm` and `sys/keys` run without a gap through the kernel vehicles, and `proc/prctl` checks `no_new_privs` per thread; `native_raw::prctl_refusals_answer_or_stop_by_name` runs strict mode's named fatal end to end from both doors; shim unit tests hold the filter's and the enforcement's privilege checks to their declared capability, the install and the enforcement to their named fatals, the ruleset attribute's copy to its order, `CHOWN` to its capability, and the key quota, displacement and collectors to the kernel's rules; `sys/keys` checks the keyring per thread (a thread older than it: `ENOKEY`, `EACCES` reading its key, its own new keyring and key; a newer thread reading it).
...
- one trace format version: only the current format is written or read, and a bundle declaring any other version is refused as unsupported;
...
- self-contained fault replay (format 4): a record run stores its fault configuration in trace metadata so replay needs no knobs re-supplied; the recorded config is authoritative. Fresh-process filesystem crash restart is native-only: native `--fs-crash-at` and torn granularity record and replay across both incarnations, Cargo/WASI refuse them, and campaigns do not draw them. Other fs error/short-I/O/latency, sleep/net faults and base latency retain flag-free replay. The `replay` subcommand rejects re-supplied fault knobs up front;
- trace lifecycle protocol (format 5): operation events carry a global order and incarnation id, and timelines carry lifecycle markers in the same order namespace;
...
4. **Self-contained replay and fail-closed fingerprint.** The realized
   ...
   metadata (additive `buggify` field; conflicting
...
The shadow-stack codes answer as 6.8 does on a CPU without `USER_SHSTK`
(`ARCH_SHSTK_STATUS` reports no feature, `ARCH_SHSTK_LOCK` records the lock,
enabling or disabling a feature is `EOPNOTSUPP` after the lock and one-feature
checks). The codes 6.8 knows but patina does not model (the xstate permissions,
mapping a vDSO) are named fatals, and the rest `EINVAL`. `modify_ldt`
...
so a signal through init's pidfd is `EPERM`: init is root's, as the pinned
oracle's pid 1 is. Its 6.8 fallback to a `/proc/<pid>` directory
...
without a gap, including the checks added for a
...
a flag, the vector (`import_iovec`), the descriptor, an advice outside the
non-destructive set, init's `mm_access` (`EACCES` without `CAP_SYS_PTRACE`),
...
The futex2 rows (`futex_wake`, `futex_wait`, `futex_requeue`, `futex_waitv`)
are modeled on the futex queues the multiplexed `futex` row parks on
(`src/thread/futex2.rs`). Each queued waiter is an entry carrying its key's
privacy, its bitset and, for a futex2 wait, its index. A futex2 wake matches
waiters by the word's address and privacy and by a shared bit, wakes one for
a negative count, and reports the highest index it unqueued to a
`futex_waitv` waiter. `futex_waitv` parks on all its words at once through the
multi-location park every blocking row uses, so its other entries leave their
queues when one wakes it, and a requeued entry moves its place in the wait
with it. Deadlines are absolute on the virtual `CLOCK_MONOTONIC` or
`CLOCK_REALTIME`: a deadline that passes while the task is parked answers
`ETIMEDOUT` even if a word changed meanwhile, and one already past answers
`ETIMEDOUT` once every word still holds its value. A handler ends a futex2 wait
with `ERESTARTSYS`, so under `SA_RESTART` the whole call runs again. The
validation follows 6.8's order. A word's key is its address in the one
process, so two addresses mapping one shared page are two keys here. The
multiplexed row's waiters and wakes carry their key and bitset the same way
(`FUTEX_WAIT_BITSET` and `FUTEX_WAKE_BITSET` take `val3`, where 0 is
`EINVAL`), its wake of a count of 0 or below wakes one (it is not
`FLAGS_STRICT`), and its `FUTEX_REQUEUE` and `FUTEX_CMP_REQUEUE` are
`futex_requeue`'s model. A wait's outcome is read before a handler that came
pending runs, as the kernel returns it first. `thread/futex2` passes with no
gap; the shim's `futex2` unit tests pin the parked deadline, the restart, a
