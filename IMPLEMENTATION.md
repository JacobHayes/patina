...
- self-contained fault replay (format 4): a record run stores its full fault configuration (crash point + torn granularity, fs error/short-I/O rates, fs latency, sleep/net jitter, drop, base net latency) in the trace metadata, so a replay reproduces the faults with no knobs re-supplied; the recorded config is authoritative, and a pre-format-4 trace keeps the historical re-supply behavior. The `replay` subcommand rejects a re-supplied fault knob up front on every family (the trace is authoritative); the runtime-level reconcile still fails closed on any conflicting knob supplied through the `run`/`test` replay path;
