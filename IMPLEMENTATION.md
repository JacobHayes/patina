...
3. Syscall-user-dispatch slices 2–3: the full FS/network dispatch rows and the committed rustix-default testbed (slice 2), and arm64 enablement once generic-entry kernels ship (slice 3, a probe flip — the number table is already arch-complete).
...
- native audit/run blocker resolution (three coupled gate fixes):
  ...
  3. **Named classes for two silent-failure surfaces, gate stays default-deny.** (a) macOS CoreFoundation/Security symbols (`CF*`/`kCF*`/`Sec*` — the `rustls-native-certs` / native TLS trust-root surface) were bare `unknown-import`; they now classify as `macos-framework`, and the run refusal carries a determinism note naming the host-keychain/trust-store non-reproducibility and the `--allow-unsupported-symbols` allow path with its qualified-determinism caveat. (b) A custom `#[global_allocator]` deadlocks before `main` under the shim (the synchronization interposers allocate through the global allocator; a custom allocator whose lazy init takes an interposed lock re-enters it — root-caused on tikv-jemallocator: `malloc_init_hard` → `os_unfair_lock` → shim interposer → `malloc` → `malloc_init_hard`, a silent pre-main hang). `native_binary_installs_custom_global_allocator` detects the class STATICALLY (a shim-linked binary that defines `__rust_alloc` but omits the default-allocator `__rdl_alloc` shim) and BOTH `run` and `audit` refuse it before the process is spawned with a named `custom-global-allocator` diagnostic; `--allow-unsupported-symbols all` is the blanket "run anyway, determinism unqualified" hatch (a narrower named list does not hatch it). This converts the dominant pre-main-hang cause from a silent sweep-eating hang into a named pre-spawn refusal. (A fully-general "no runtime op within N wall-seconds of exec" pre-main watchdog would need a guest-liveness heartbeat from the shim to stay false-positive-free against legitimately long runs; deferred — see VALIDATION.md.)
