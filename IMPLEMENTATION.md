...
3. Syscall-user-dispatch (Linux ≥5.11, x86_64 first — the aarch64 enablement is still out-of-tree): trap raw inline syscalls via SIGSYS into the same deterministic routes so raw-syscall crates run in-model instead of being refused; design settled (allowed-region = glibc text, zero selector toggles, vDSO closed by auxv scrub), sequenced behind nothing but implementation time.
