...
The full landing battery runs the frozen signals-family gate as a parallel rung
with its own build and conformance-output directory under `target/check/parallel/`.
...
including the cargo-patina termination/core-envelope e2e tests. The frozen family
gate and the full landing battery are the whole-family acceptance criteria.
