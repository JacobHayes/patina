...
- self-contained fault replay (format 4): a record run stores its fault configuration in trace metadata so replay needs no knobs re-supplied; the recorded config is authoritative, and a pre-format-4 trace keeps the historical re-supply behavior. The current exception is fresh-process filesystem crash restart: `--fs-crash-at` and torn granularity are native seeded-run only, native record/replay and Cargo/WASI refuse them, and campaigns do not draw them. Other fs error/short-I/O/latency, sleep/net faults and base latency retain flag-free replay. The `replay` subcommand rejects re-supplied fault knobs up front;
