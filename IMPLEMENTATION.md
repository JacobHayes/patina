...
- self-contained fault replay (format 4): a record run stores its full fault configuration (crash point + torn granularity, sleep/net jitter, drop, base net latency) in the trace metadata, so `--replay` reproduces the faults with no knobs re-supplied; the recorded config is authoritative, conflicting explicit knobs fail closed at build time, and a pre-format-4 trace keeps the historical re-supply behavior;
