...
Both halves of that link come from one toolchain, and a build that would split them is refused before it compiles anything. The shim's Cargo build is pinned to the Patina source workspace so `-p patina-dst-native-shim` always resolves, while the guest builds in the caller's working directory; under rustup, `rustc` is a proxy that picks its toolchain from the rust-toolchain file above whichever directory it runs in. A guest tree carrying its own pin, reached by invoking the `cargo-patina` binary directly instead of as `cargo patina` (the cargo proxy exports `RUSTUP_TOOLCHAIN` for the whole build), therefore compiles the two halves with different compilers, and two Rust standard libraries meet at the guest link — `duplicate symbol: rust_eh_personality` on Linux, and on macOS a silent success producing a guest that carries both. The build compares the rustc identity that resolves in each directory and refuses, naming both toolchains, the directory each resolved in, and the two ways to pin one toolchain for both.
...
Each call is a recorded boundary operation (`Operation::CustomOp`) whose outcome is the result bytes, so a replay whose custom-op stream diverges — a changed label, a changed key, a missing or extra call — is refused by name rather than answered from a recording of a different question. Every custom-op refusal is fatal: the native shim emits `PATINA_CUSTOM_OP_REFUSED` and aborts, and the WASI host traps, because none of them has an answer the guest could safely be handed.
