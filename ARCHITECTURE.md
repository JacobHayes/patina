...
Both halves of that link come from one toolchain, and a build that would split them is refused before it compiles anything. The shim's Cargo build is pinned to the Patina source workspace so `-p patina-dst-native-shim` always resolves, while the guest builds in the caller's working directory. When `rustc` is a rustup proxy, it picks its toolchain from the rust-toolchain file above whichever directory it runs in; a guest tree carrying its own pin, reached by invoking the `cargo-patina` binary directly instead of as `cargo patina` (the cargo proxy exports `RUSTUP_TOOLCHAIN` for the whole build), can therefore compile the two halves with different compilers, and two Rust standard libraries meet at the guest link — `duplicate symbol: rust_eh_personality` on Linux, and on macOS a silent success producing a guest that carries both. The build compares the rustc identity that resolves in each directory and refuses, naming both toolchains and the directory each resolved in. Rustup remediation applies when the active `rustc` is the rustup proxy; the proxy-agnostic remedy is to invoke `cargo-patina` with absolute, matching `RUSTC` and `CARGO` binaries from one toolchain. Its internal shim cache is also keyed by the complete `rustc -vV` identity, so concurrent stable/MSRV supervisors cannot publish same-named archives into one default path; an explicit `CARGO_TARGET_DIR` remains authoritative.
