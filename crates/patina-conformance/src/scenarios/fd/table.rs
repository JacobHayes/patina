...
//! /dev/null), and the non-file kinds' answers to lseek.
//!
...
pub fn run(p: &Probe) {
    ...
    p.check(
        "dup3 of equal numbers is EINVAL",
        p.dup3(a, a, 0) == neg(EINVAL),
    );
    p.check(
    ...
    p.check(
        "a write to number 2 lands in the file",
        p.write(2, b"redirected") == 10,
    );
    p.check("restore standard error", p.dup2(saved, 2) == 2);
    p.check("rewind the file", p.lseek(out, 0, SEEK_SET) == 0);
    let (n, data) = p.read(out, 16);
    p.check(
        "the file holds the redirected bytes",
        n == 10 && data == b"redirected",
    );
    ...
    // ---- EBADF ----
    p.check(
        "close of a closed number is EBADF",
        p.close(4000) == neg(EBADF),
    );
    p.check(
        "close of a negative number is EBADF",
        p.close(-1) == neg(EBADF),
    );
    p.check(
