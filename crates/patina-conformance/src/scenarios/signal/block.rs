...
pub fn run(p: &Probe) {
    ...
    thread::scope(|scope| {
        support::delayed_kills(scope, p, pid, &[SIGUSR2]);
        p.check(
    ...
    p.check(
        ...
        ssi.is_some_and(|ssi| ssi.ssi_signo == SIGUSR2 as u32 && ssi.ssi_code == SI_USER),
    );
    p.check(
    ...
    thread::scope(|scope| {
        support::delayed_kills(scope, p, pid, &[SIGUSR2, SIGUSR1]);
        p.check(
