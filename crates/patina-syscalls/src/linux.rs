...
pub const fn disposition(id: Syscall) -> SyscallRow {
    ...
    Syscall::N_futex => r(
        ...
        Family::Sync,
        Disposition::Modeled,
        "FUTEX_WAIT/WAKE and their BITSET forms park and wake on the deterministic scheduler's futex queues by key and bitset (the same decode as the libc `syscall(2)` interposer, timeouts on the virtual clock; an empty bitset is EINVAL, a wake count of 0 wakes one), and FUTEX_REQUEUE/FUTEX_CMP_REQUEUE move waiters (`thread::futex2`); other ops answer ENOSYS.",
        None,
    ),
    ...
    Syscall::N_add_key => r(
        ...
        Family::Privileged,
        Disposition::Modeled,
        "The guest's own keys (`sud::privileged::keys`): process keyrings, each the credential state of the thread that made it and of the threads created after, made on demand, holding `user` keys, with deterministic serials; a session keyring outside the model. In `add_key`'s order: a payload of a megabyte or more `EINVAL`; the type (`EFAULT`, empty or 32 bytes `EINVAL`, internal `EPERM`); the description (`EFAULT`, `EINVAL` past 4096 bytes, a keyring's leading dot `EPERM`); the payload (`EFAULT`); the ring (`EINVAL`, `ENOKEY`, `EKEYREVOKED`, `EACCES` without write permission: the calling thread does not possess it); the type (`ENODEV` for none the pinned kernel registers); a ring that is no keyring `ENOTDIR`; the payload's size and a description (`EINVAL`); then a live key of the same description is updated in place, or a new one is charged to the quota (`KERNEL_CONFIG.keys_maxkeys`/`keys_maxbytes`: `EDQUOT`) and linked. Named fatals: the other registered types, a keyring inside a keyring, the thread, session and user keyrings, a keyring its last thread left and the keys in it.",
        None,
    ),
    Syscall::N_request_key => r(
        ...
        Family::Privileged,
        Disposition::Modeled,
        "The type, description and callout strings, the destination (made on demand), the type (`ENOKEY` for an unregistered one), then a search of the calling thread's process keyring for a live `user` key, and of the session keyring, which holds nothing the guest can find; a miss without callout information is `ENOKEY` (a revoked key's description included, as on a host whose process has a session keyring). Named fatals: the upcall a callout would start, a keyring type, the other registered types.",
        None,
    ),
    Syscall::N_keyctl => r(
        id,
        Family::Privileged,
        Disposition::Modeled,
        "The operation is an `int`: past 32 `EOPNOTSUPP`. Modeled: `GET_KEYRING_ID` (`ENOKEY` for a thread keyring not asked to be made, and for a process keyring the calling thread does not hold), `UPDATE`, `REVOKE` (a revoked key then reads, describes and looks up as `EKEYREVOKED`), `CHOWN` (another user, or a group the caller is not in, needs `CAP_SYS_ADMIN`: `EACCES`), `DESCRIBE` (`type;uid;gid;perm;description`), `READ` (the length, copied only when it fits) and `CAPABILITIES` (the pinned configuration's two bytes). A key the calling thread does not possess answers through `key_task_permission` (`EACCES`). Named fatals: the other operations, revoking a keyring, reading a keyring of more than one key, a keyring its last thread left and the keys in it, and a revoked key past `kernel.keys.gc_delay`, when the collector would remove it.",
        None,
    ).capabilities(&[Capability::SysAdmin]),
    ...
    Syscall::N_prlimit64 => r(
        ...
        Family::Identity,
        Disposition::Modeled,
        "`prlimit64` of the guest (0, its pid or one of its threads; any other pid `ESRCH`, and init, root's, `EPERM` to read or write, `check_prlimit_permission`) answers from one table of the virtual kernel's resource limits (`patina_prlimit`, the C `getrlimit`/`setrlimit` too; `src/limits.rs`): all 16 resources start at the kernel's `INIT_RLIMITS` on a 4 GiB machine (`RLIMIT_NOFILE` 1024/4096, `RLIMIT_MEMLOCK` 8 MiB, `RLIMIT_MSGQUEUE` 819200, `RLIMIT_STACK` 8 MiB/unlimited, `RLIMIT_CORE` 0/unlimited, `RLIMIT_NPROC` and `RLIMIT_SIGPENDING` 16384, `RLIMIT_NICE`/`RLIMIT_RTPRIO` 0, the rest unlimited) and follow `do_prlimit`'s unprivileged rule: lowered freely, soft raised to hard, a hard raise `EPERM` (`RLIMIT_NOFILE` past `nr_open` `EPERM`, soft above hard `EINVAL`). Enforced for the guest: `RLIMIT_NOFILE` (the descriptor table's `EMFILE` bound, `poll`'s size bound), `RLIMIT_MEMLOCK` (lock accounting), `RLIMIT_MSGQUEUE` (POSIX queues); the others are kept and reported only.",
        None,
    ),
    ...
    Syscall::N_pkey_mprotect => r(
        ...
        Family::Mem,
        Disposition::Modeled,
        "The virtual CPU has no protection keys, on every host (keys are CPU state a guest reaches without a syscall, `rdpkru`/`wrpkru`, so no host's keys could answer the same everywhere): x86_64 answers as 6.8 without `OSPKE`, where no key is ever allocated to a user interface: `mprotect` for key -1, `EINVAL` for any other key after `do_mprotect_pkey`'s earlier checks; aarch64's 6.8 does not build `ARCH_HAS_PKEYS`, `ENOSYS`.",
        None,
    ),
    Syscall::N_pkey_alloc => r(
        ...
        Family::Mem,
        Disposition::Modeled,
        "The virtual CPU has no protection keys, on every host (keys are CPU state a guest reaches without a syscall, `rdpkru`/`wrpkru`, so no host's keys could answer the same everywhere): x86_64 answers as 6.8 without `OSPKE`, `EINVAL` for a flag or unknown rights; otherwise the first call takes key 0, which the map starts without, and fails `EINVAL` (`arch_set_user_pkey_access`) with key 0 left taken, and every later one is `ENOSPC`; aarch64's 6.8 does not build `ARCH_HAS_PKEYS`, `ENOSYS`.",
        None,
    ),
    Syscall::N_pkey_free => r(
        ...
        Family::Mem,
        Disposition::Modeled,
        "The virtual CPU has no protection keys, on every host (keys are CPU state a guest reaches without a syscall, `rdpkru`/`wrpkru`, so no host's keys could answer the same everywhere): x86_64 answers as 6.8 without `OSPKE`, where no key is ever allocated to a user interface, so every call is `EINVAL`; aarch64's 6.8 does not build `ARCH_HAS_PKEYS`, `ENOSYS`.",
        None,
    ),
    Syscall::N_statx => r(
        id,
    ...
    Syscall::N_landlock_create_ruleset => r(
        ...
        Family::Privileged,
        Disposition::Modeled,
        "Any caller may build a ruleset (`sud::privileged::landlock`): a flag answers the ABI version (`KERNEL_CONFIG.landlock_abi`, 4) or, Ubuntu's backport, the errata (5), only with no attribute, else `EINVAL`; the attribute's copy (`copy_min_struct_from_user`: NULL `EFAULT`, short of `handled_access_fs` `EINVAL`, past a page `E2BIG`, a nonzero tail past the known structure `E2BIG`), a right ABI 4 does not know (`EINVAL`), none (`ENOMSG`); then a descriptor-table kind (`FdKind::LandlockRuleset`, the anonymous `[landlock-ruleset]` inode: read-write, close-on-exec, read and write `EINVAL`, no seek, no poll method).",
        None,
    ),
    Syscall::N_landlock_add_rule => r(
        ...
        Family::Privileged,
        Disposition::Modeled,
        "A flag (`EINVAL`), then the ruleset (`EBADF`, `EBADFD` for another kind), then the rule type (`EINVAL`); a path rule's attribute (`EFAULT`), rights (`ENOMSG` for none, `EINVAL` for one the ruleset does not handle), then its parent (`fdget_raw`: `EBADF`; a ruleset, or a node on an internal filesystem or mount such as a pipe's or a memfd's, `EBADFD`); a port rule's attribute, rights and port (past 65535 `EINVAL`). Rules are accepted, not kept: nothing enforces them. A captured standard stream as a parent is a named fatal (it has no modeled node).",
        None,
    ),
    Syscall::N_landlock_restrict_self => r(
        ...
        Family::Privileged,
        Disposition::Modeled,
        "Without `no_new_privs`, `CAP_SYS_ADMIN` (`EPERM`) before the flags and the descriptor; then a flag (`EINVAL`) and the ruleset (`EBADF`, `EBADFD`). Enforcing a ruleset is a named fatal: patina does not enforce Landlock.",
        None,
    ).capabilities(&[Capability::SysAdmin]),
    ...
    Syscall::N_futex_waitv => r(
        ...
        Family::Sync,
        Disposition::Modeled,
        "Parks the caller on up to 128 32-bit words at once on the scheduler's futex queues (`thread::futex2`), until a wake of one (answering its index), an absolute `CLOCK_MONOTONIC`/`CLOCK_REALTIME` virtual deadline (`ETIMEDOUT`) or a handler (restarted under `SA_RESTART`); a stale word is `EAGAIN`, and the flags, sizes, values and clock are checked in 6.8's order (`EINVAL`, `EFAULT`).",
        None,
    ),
    ...
    Syscall::N_map_shadow_stack => r(
        ...
        Family::Mem,
        Disposition::Modeled,
        "The virtual CPU has no user shadow stacks, on every host, so the answers do not depend on whether the host's CPU has `X86_FEATURE_USER_SHSTK`: x86_64 answers `EOPNOTSUPP`, as 6.8 does without the feature, before any argument; aarch64's 6.8 has no shadow stacks, `ENOSYS`.",
        None,
    ),
    Syscall::N_futex_wake => r(
        ...
        Family::Sync,
        Disposition::Modeled,
        "Wakes up to `nr` waiters (one when negative) queued by the word's address and privacy whose bitset shares a bit with the mask, on the scheduler's futex queues (`thread::futex2`); a size other than 32 bits, an unknown flag, an empty or too-wide mask and a misaligned word are `EINVAL`.",
        None,
    ),
    Syscall::N_futex_wait => r(
        ...
        Family::Sync,
        Disposition::Modeled,
        "Parks the caller on a 32-bit word with its bitset on the scheduler's futex queues (`thread::futex2`), until a matching wake (0), an absolute `CLOCK_MONOTONIC`/`CLOCK_REALTIME` virtual deadline (`ETIMEDOUT`) or a handler (restarted under `SA_RESTART`); a stale word is `EAGAIN`, and the flags, value, mask and clock are checked in 6.8's order (`EINVAL`, `EFAULT`).",
        None,
    ),
    Syscall::N_futex_requeue => r(
        ...
        Family::Sync,
        Disposition::Modeled,
        "Compares the first word's value (`EAGAIN`), then wakes `nr_wake` of its waiters by key and moves the next `nr_requeue` to the second word's key with their bitsets, on the scheduler's futex queues (`thread::futex2`); a flag or a negative count is `EINVAL`.",
        None,
    ),
