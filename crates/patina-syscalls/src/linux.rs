...
pub const fn disposition(id: Syscall) -> SyscallRow {
    ...
    Syscall::N_migrate_pages => r(
        ...
        Family::Mem,
        Disposition::Modeled,
        "Nothing moves on one node: 0 for the process itself, `EPERM` for a target node outside the allowed one, `ESRCH` for another pid.",
        None,
    ),
    Syscall::N_openat => r(
        id,
    ...
    Syscall::N_move_pages => r(
        ...
        Family::Mem,
        Disposition::Modeled,
        "Every page is on node 0: a query answers 0, `-ENOENT` for a mapped page not resident, `-EFAULT` for an unmapped one; a move to node 0 succeeds and to any other node is `ENODEV`; another pid is `ESRCH`. Gap: residency is the host's `mincore`, so a page only ever read (the kernel's shared zero page, which `move_pages` answers `-EFAULT`) reads as node 0, and host reclaim can evict a touched page.",
        None,
    ),
    ...
    Syscall::N_prlimit64 => r(
        ...
        Family::Identity,
        Disposition::Modeled,
        "`prlimit64` of the virtual process (pid 0 or 1; any other `ESRCH`): the virtual kernel's resource limits (`patina_prlimit`, the C `getrlimit`/`setrlimit` too; `src/limits.rs`): all 16 resources start at the kernel's `INIT_RLIMITS` on a 4 GiB machine (`RLIMIT_NOFILE` 1024/4096, `RLIMIT_MEMLOCK` 8 MiB, `RLIMIT_MSGQUEUE` 819200, `RLIMIT_STACK` 8 MiB/unlimited, `RLIMIT_CORE` 0/unlimited, `RLIMIT_NPROC` and `RLIMIT_SIGPENDING` 16384, `RLIMIT_NICE`/`RLIMIT_RTPRIO` 0, the rest unlimited) and follow `do_prlimit`'s unprivileged rule: lowered freely, soft raised to hard, a hard raise `EPERM` (`RLIMIT_NOFILE` past `nr_open` `EPERM`, soft above hard `EINVAL`). Enforced: `RLIMIT_NOFILE` (the descriptor table's `EMFILE` bound, `poll`'s size bound), `RLIMIT_MEMLOCK` (lock accounting), `RLIMIT_MSGQUEUE` (POSIX queues); the others are kept and reported only.",
        None,
    ),
