...
| — | **Host introspection (macOS Mach/BSD/IOKit)** | `sysctl`/`sysctlbyname`, `getrusage`, `task_info`, `mach_task_self_`, `mach_host_self`, `host_statistics64`, `host_processor_info`, `vm_page_size`, `vm_deallocate`, `proc_listallpids`/`proc_pidinfo`/`proc_pid_rusage`/`proc_pidpath`; IOKit `IOServiceMatching`/`IOServiceGetMatchingServices`/`IOIteratorNext`/`IOObjectRelease`/`IORegistryEntryCreateCFProperty`/`IORegistryEntryGetName` — the `sysinfo` / `num_cpus` / hardware-inventory surface | **not interposed → refused.** These read host CPU/memory/hardware/process state — core counts, memory pressure, thermal/battery/device inventory, per-process resource usage — nondeterministic across hosts and runs, so a run reaching one is not reproducible: **interpose-or-refuse, never allowlist.** Classified `host-introspection` (the exact Mach/BSD name list plus the IOKit entry-point prefixes `IOService`/`IORegistry`/`IOIterator`/`IOObject` — deliberately not a bare `IO`, which would capture arbitrary user `IO*` symbols) as a named refinement of the bare `unknown-import` these fell to, so the run refusal carries a determinism note and the `--allow-unsupported-symbols` path (qualified determinism). Fail-closed either way: the refinement only sharpens an already-denied label, never relaxes a decision. | import audit → `host-introspection` | `classifies_ecosystem_audit_symbol_batch` (unit — representative Mach/BSD/IOKit sample classifies; `sysctlbyname` stays denied; a user `IOWidget` does NOT match; `IO`-prefix overreach guarded); the sysinfo/mimalloc MREs audit with the class attached |
