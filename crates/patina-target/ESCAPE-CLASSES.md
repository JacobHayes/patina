...
| — | **Environment** | `getenv`, `secure_getenv`, `setenv`, `unsetenv`, `putenv` | interposed → deterministic environment map supplied by native `run --env` (empty by default, recorded in trace metadata); mutation stays fail-closed | symbol audit → `environment` | `every_escape_class_is_detected_and_denied` |
