...
| c | **Entropy** | `getentropy`, `getrandom`, `arc4random*`, `CCRandomGenerateBytes`, `SecRandomCopyBytes`; `/dev/urandom` fallback reads | interposed or modeled device → seeded RNG | symbol audit → `entropy` for raw imports; deterministic FS/device path for `/dev/urandom` | `every_escape_class_is_detected_and_denied`; `validate-native-shim.sh` `rand::rng()` and `/dev/urandom` determinism legs |
