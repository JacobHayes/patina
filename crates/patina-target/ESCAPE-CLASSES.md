...
| b | **Time** | `clock_gettime`, `clock_gettime_nsec_np`, `gettimeofday`, `mach_absolute_time`, `mach_continuous_time`, `nanosleep`, `clock_nanosleep`, `usleep`, `mach_wait_until` | all interposed → virtual clock (`clock_gettime_nsec_np` shares `clock_gettime`'s clock-id mapping, returning nanoseconds directly) | symbol audit → `time` | `every_escape_class_is_detected_and_denied`; `validate-native-shim.sh` clock_gettime_nsec_np e2e |
...
| — | **macOS system frameworks (CoreFoundation / Security)** | `CFArrayCreate`/`CFStringGetLength`/`CFDataGetBytePtr`/`kCFAllocatorDefault`/`kCFTypeArrayCallBacks`; `SecCertificateCopyData`/`SecTrustSettingsCopyCertificates`/`SecCopyErrorMessageString` — the `rustls-native-certs` / `security-framework` / native TLS trust-root surface | **not interposed → refused.** The Security-framework subset reads the host keychain / system trust store — mutable per-machine, per-time host state — so a run reaching it is not reproducible; the CoreFoundation helpers are the data-structure plumbing those calls require. Classified `macos-framework` (Apple-reserved prefixes `CF`/`kCF`/`Sec`/`kSec`) as a named refinement of the bare `unknown-import` these fell to, so the run refusal can attach a determinism note naming the host-trust-store problem and the `--allow-unsupported-symbols` allow path (qualified determinism). Fail-closed either way: the prefix match only sharpens an already-denied label, never relaxes a decision. | import audit → `macos-framework` | `native_gate_classifies_and_refuses_a_security_framework_symbol` (macOS e2e — refusal + note + audit/run parity); `classifies_known_native_escape_symbols` (unit — the 21-symbol certs surface) |
| — | **Custom global allocator (pre-main deadlock)** | a guest `#[global_allocator]` (e.g. `tikv_jemallocator::Jemalloc`) | **refused before spawn.** The shim's synchronization interposers allocate through the global allocator (the lock table registers each lock lazily on first touch), so a custom allocator whose OWN lazy init takes an interposed lock re-enters the half-initialized allocator while the shim holds its non-reentrant runtime lock and DEADLOCKS before `main` — a silent pre-main hang (root-caused on jemalloc: `malloc_init_hard` → `os_unfair_lock` → shim interposer → `malloc` → `malloc_init_hard`). Detected STATICALLY by a defined-symbol scan — a shim-linked binary defines `__rust_alloc` but omits the default-allocator `__rdl_alloc` shim — and BOTH `run` and `audit` refuse it with a named `custom-global-allocator` diagnostic. `--allow-unsupported-symbols all` is the blanket "run anyway, determinism unqualified, may hang" hatch; a narrower named list does not hatch it (there is no symbol name). Structural, not behavioral: refuses ALL custom global allocators because safe-vs-deadlocking is not statically decidable and the prevented failure is a silent hang. | **defined-symbol scan** (`native_binary_installs_custom_global_allocator`), pre-spawn — NOT the import audit | `native_run_and_audit_refuse_a_custom_global_allocator` (e2e — default refuse / narrow-list still refuse / audit refuse / blanket-`all` warns+runs a System-wrapper); `custom_global_allocator_detection_fails_closed_on_unparseable_input` (unit — fail-closed on unparseable input) |
