...
3. **Instruction-level entropy / time** (`rdrand`/`rdseed`/`rdtsc`/`rdtscp` on
   x86_64, `mrs CNTVCT_EL0` and `RNDR` on aarch64, raw `svc`/`syscall`).
   `scan_instruction_classes` decodes all of these except aarch64 `RNDR`/`RNDRRS`,
   which it does not decode yet: `rdtsc`/`rdtscp` are trap-managed on x86-64
   Linux, `rdrand`/`rdseed`/`CNTVCT` refuse everywhere, and `cpuid` is reported
   visible-not-refused (host-identity). Thread-pointer writes (`wrfsbase`, FS
   selector loads, `msr tpidr_el0`) are refused in the same scan. The residual is
   encodings the decoder does not know (`RNDR` among them), which refuse only when
   they sit at a decode boundary the scan reaches; code outside the scanned
   image: the scan reads the guest executable, not the shared libraries it links
   ...
   no trap backs (thread-pointer writes, `rdrand`/`rdseed`/`CNTVCT`) that last
   case has no run-time backstop either.
