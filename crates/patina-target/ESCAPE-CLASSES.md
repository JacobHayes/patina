...
3. **Instruction-level entropy / time** (`rdrand`/`rdseed`/`rdtsc`/`rdtscp` on
   ...
   The residual is encodings the decoder does not know, which refuse only when
   ...
   no trap backs (thread-pointer writes, `rdrand`/`rdseed`/`CNTVCT`/`CNTVCTSS`/
   `RNDR`) that last case has no run-time backstop either.
