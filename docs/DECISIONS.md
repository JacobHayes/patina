...
| 4 | Native run clears guest env; `setenv` fails closed | arch + fix | Empty-by-default deterministic env with host env excluded is by design (host env is a nondeterminism source); explicit `--env KEY=VALUE` injects values, recorded in trace metadata. REVISED per user (2026-08-06): guest-side mutation is supported — `setenv`/`unsetenv`/`clearenv`/`putenv` are glibc's functions over the process's own `environ`, which starts as the startup `--env` array (no per-mutation trace records — only the startup `--env` set is metadata); `putenv` keeps the caller's string aliased, as glibc does, once `environ` rather than an owned map became the store. |
...
| 12 | `campaign --faults` doesn't choose fs crash points | open: crash band not built | Native `--fs-crash-at` has fresh-incarnation semantics and records and replays across both incarnations (`docs/fs-crash-restart-protocol.md`), so campaign failure artifacts can carry a crash. Campaign generation still draws no fs crash/torn knobs. Drawing them needs a native-only crash band; a crash point every generation's workload reaches (an armed selector that is never reached is a named runtime error); classifier awareness of `PATINA_FS_CRASH_SELECTOR_UNREACHED`, `PATINA_FS_CRASH_REPLAY_DIVERGENCE` and `PATINA_FS_CRASH_INVALID_HANDOFF`; and `minimize` verified on two-incarnation traces. |
