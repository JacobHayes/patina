...
| 12 | `campaign --faults` doesn't choose fs crash points | paused pending lifecycle replay | Native seeded `--fs-crash-at` now has fresh-incarnation semantics, but campaign generations rely on record/replay for failure artifacts and minimization. Until v5 crash lifecycle replay is wired, fs crash/torn knobs stay out of campaign generation and are exercised only by explicit native seeded runs. |
