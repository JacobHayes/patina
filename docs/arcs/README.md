...
| [syscall-conformance](syscall-conformance.md) | Registry-as-code over the vendored kernel syscall tables (every x86_64 number dispositioned, `cargo patina syscalls`), live host-oracle conformance scenarios (three vehicles, typed event comparison, exact declared gaps), and the build-out of every modelable family: unified fd table, cwd, fs extensions, IPC, timers/identity, faithful signals, SimNet expansion. |
| [syscall-conformance-signals](syscall-conformance-signals.md) | The signals + threads + process family: per-row Linux semantics with kernel citations, the scheduler-integrated delivery design at file:line (per-task state, generation, four delivery points, kernel-built frames via self-directed `rt_tgsigqueueinfo`, the restart rule, termination by the real signal, `SignalGenerated`), the suggested order of work M1–M5 with its scenarios, required unit tests and trace facts. |
