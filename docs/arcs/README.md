...
- **Phase-2 items belong to their arc.** Anything a doc labels "phase 2" is a scheduled
  later wave of that same arc (coverage-guided exploration is coverage-depth Wave E,
  static site enumeration is invariant-visibility Wave 5) — never an unowned
  someday-item. One deliberate exception: point-solution's harness-fixtures sketch is
  unscheduled by user call (no concrete consumer yet).
...
| [syscall-conformance](syscall-conformance.md) | Registry-as-code over the vendored kernel syscall tables (every x86_64 number dispositioned, `cargo patina syscalls`), a host-oracle conformance testbed (three vehicles, typed event diffs, per-(os,arch) expectations), and the build-out of every modelable family: unified fd table, cwd, fs extensions, IPC, timers/identity, faithful signals, SimNet expansion. |
| [syscall-conformance-signals](syscall-conformance-signals.md) | The signals + threads + process family: per-row Linux semantics with kernel citations, the scheduler-integrated delivery design at file:line (per-task state, generation, four delivery points, kernel-built frames via self-directed `rt_tgsigqueueinfo`, the restart rule, termination by the real signal, `SignalGenerated`), the suggested order of work M1–M5 with its probes, required unit tests and trace facts, and the frozen oracle + its one gate, `gate.sh --family signals`. |
