...
| `futex_wake`, `futex_wait`, `futex_requeue`, `futex_waitv` | 32-bit words only (another size, an unknown flag, a too-wide value or mask, an empty mask, a misaligned word: `EINVAL`), keyed by address and privacy; a wake matches waiters by key and by a shared bitset bit, and wakes one for a negative count; `futex_requeue` compares the first value (`EAGAIN`), wakes `nr_wake` whatever their bitset and moves the next `nr_requeue` to the second key; `futex_waitv` waits on up to 128 words and answers the highest index a wake unqueued; absolute `CLOCK_MONOTONIC`/`CLOCK_REALTIME` deadlines (another clock `EINVAL`) answer `ETIMEDOUT`; a handler restarts the call under `SA_RESTART` (`ERESTARTSYS`) | `kernel/futex/syscalls.c`, `waitwake.c`, `requeue.c` | `thread/futex2` |
| `kcmp(pid1, pid2, type, idx1, idx2)` | both pids looked up (`ESRCH`), init `EPERM` (`CAP_SYS_PTRACE`) before the type; the guest's threads share `KCMP_VM`/`FILES`/`FS`/`SIGHAND`/`SYSVSEM` (0) and each has its own I/O context once `ioprio_set` gives it one or it is created by a thread with a valid priority (no `CLONE_IO`); `KCMP_FILE` compares descriptions (`O_PATH` included, `unsigned int` index, `EBADF`); `KCMP_EPOLL_TFD`: slot `EFAULT`, then `idx1` `EBADF`, `efd` `EBADF`/`EINVAL`, then `ENOENT`; distinct objects answer 1 or 2 by a deterministic order; another type `EINVAL` | `kernel/kcmp.c`, `copy_io`, `set_task_ioprio` | `proc/kcmp` |
