...
|---|---|---|---|
| `kill(pid, sig)` | `pid > 0` the process, `0` the caller's process group, `-1` every signalable process (all three are the one virtual process); `sig == 0` probes (delivers nothing); `sig < 0` or `> 64` → `EINVAL`; an absent pid → `ESRCH`. A signal to self is delivered before `kill` returns (the pending signal is handled on the return to user mode) with `si_code = SI_USER`, `si_pid`/`si_uid` the sender's (virtual pid 1, uid 1000). | `kill_something_info`, `group_send_sig_info`, `__send_signal_locked` | `signal/basic`, `proc/ids` |
...
| `patina-dst-trace` | `format_8_migrates_to_9` | TRACE_FORMAT_VERSION is 9; a committed format-8 fixture decodes through the MIGRATIONS chain to an equal bundle at version 9, and a format-9 trace carrying signal_generated round-trips. |
...
- Recorded traces (format ≥ 9; `signal_generated` ops, in order): `signal/basic` [10:p 10:t 10:t]; `signal/raw_action` [10:p 10:p]; `signal/queue` [34:p 34:p 34:t 10:p 10:p 34:p 34:t]; `signal/rt_order` [12:p 10:p 34:p 34:p 35:p 34:p 34:p 12:p 10:p]; `signal/unmask` [10:p 10:t 12:p 10:p]; `signal/altstack` [10:p]; `signal/per_thread` [10:p 10:t].
...
- Recorded traces (format ≥ 9; `signal_generated` ops, in order): `thread/kill` [10:t 10:t 10:t]; `thread/masks` [10:p 10:t]; `thread/pthread_kill` [10:t 10:t], libc only; `thread/tid_clear` [(none)]; `thread/main_exit` [(none)].
